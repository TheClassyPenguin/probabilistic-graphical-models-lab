...
```
The resulting structure is very similar. The only relationship missing is between "Asia" and "Tuberculosis".

We can see that both the classification and confusion matrices are the same for the learnt and true structures. Since this structure performed exactly like the true graph for the given test set, we may assume that the missed connection is not very impactful and its effect may be lost in smaller datasets. 

Assuming that the true graph is the best structure to model the relationships in the data, the hill-climbing algorithm has performed well and has almost completely modeled all the true dependencies. 
...
From now on we will continue using exact inference for our results.
...
$$ S \perp_G {A, D, E, T, X} | L, B$$. 
This must mean that 
$$S \perp_p {A, D, E, T, X} | L, B$$
